...
export function computeReminders(rentals, reviewLink) {
  ...
  for (const r of rentals) {
    ...
    if (r.dropoffDate === shiftDate(today, 1)) {
      ...
      out.delivery.push({ ...r, message: msg, sms: smsLink(r.phone, msg) });
    }
    ...
    if (r.pickupDate === shiftDate(today, 2)) {
      ...
      out.pickup.push({ ...r, message: msg, sms: smsLink(r.phone, msg) });
    }
    ...
    if (r.pickupDate === shiftDate(today, -3)) {
      ...
      out.review.push({ ...r, message: msg, sms: smsLink(r.phone, msg) });
    }
  }
  return out;
}
